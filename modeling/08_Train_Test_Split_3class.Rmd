...
```{r}
# define the number of scenes that is ~ 70% of scenes
ls9_seventyperc <- round((length(unique(ls9_filt$date)))*0.7, 0)

# get unique train-val dates
ls9_tv_dates <- ls9_filt %>% 
  pluck("date") %>% 
  unique()

# sample 60% of the dates remaining
train_dates <- sample(ls9_tv_dates, ls9_seventyperc)

# and split the t-v dataset
ls9_train <- ls9_filt %>% 
  filter(date %in% train_dates)
ls9_validate <- anti_join(ls9_filt, ls9_train)
