...
of outliers in the label dataset and whether or not we can glean anything from
the metadata to be able to pre-emptively toss out scenes when we go to apply the
...
5 labels as inconsistent. Let's do a quick sanity check to make sure that we've
...
applicable to the the application of the algorithm.
...
(`r round(nrow(LS5_CDIST)/nrow(LS5_class_analysis)*100, 1)`%) in the whole
dataset that have a cloud distance \<500m. Since this is about the same portion
of labels, I don't think this is terribly helprul.
...
cloud cover was \> 60%. The filtered dataset contains `r nrow(LS5_max_cloud)`
(`r round(nrow(LS5_max_cloud)/nrow(LS5_class_analysis)*100, 1)`%) where max was
\>75% and `r nrow(LS5_mean_cloud)`
(`r round(nrow(LS5_mean_cloud)/nrow(LS5_class_analysis)*100, 1)`%) where the
mean cloud cover was \> 60%. This is a bit more uneven of a comparison, but I
don't think this is enough to filter scenes by scene-level cloud cover.

### Training dataset implications

For the purposes of training data, I think we can throw out the data from the
1988-11-23 scene, but I don't see a reason to throw out any other outliers at
this time.
