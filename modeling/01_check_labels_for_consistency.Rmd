...
Landsat 5 and 7 have the same band info
...
LS5 <- collated_data %>% 
  ...
  select(date, mission, class, vol_init, 
         ...
         B7, 
         # re-pull columns
         starts_with("SR"), starts_with("ST"), -SR_B6, -ST_B10,
         -contains("AERO")) %>% 
...
### SR Band Inconsistencies

```{r}
make_comp_plot_LS57 <- function(user_band, ee_band, data, data_name) {
...
Look at the data that don't match
...
the ee value. Let's drop those and then look more.
...
This leaves very few (40, < 2%) of labels that are inconsistent and shouldn't be
included in the test/train set. Also, since we're not labeling saturated pixels, 
we'll drop those too. Do a quick sanity check:
...
```{r}
make_class_comparison <- function(data, data_name, band) {
...
distributed. So shapiro-wilkes first!   
